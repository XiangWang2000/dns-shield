...
switch ($Action) {
    ...
    "Install" {
        ...
        Write-Host "Installed only $TestPackage on $device. Production package: $ProductionPackage"
    }
    "Run" {
        $device = Resolve-Device
        if (-not (Get-PackagePath $device $TestPackage)) {
        ...
        try {
            ...
            try {
                & $Gradle --no-daemon --console=plain `
                    -PandroidTestBuildType=d14DeviceTest `
                    ":app:connectedD14DeviceTestAndroidTest" `
            ...
            if ($null -eq $oldSerial) { Remove-Item Env:ANDROID_SERIAL -ErrorAction SilentlyContinue }
            else { $env:ANDROID_SERIAL = $oldSerial }
        }
        ...
        if (-not $newReport) {
            if ($testExitCode -ne 0) {
                throw "D14 connected instrumentation failed with exit code $testExitCode and produced no report."
            }
            throw "D14 connected instrumentation completed without producing a fresh report."
        }
        ...
        if ($testExitCode -ne 0) {
            throw "D14 connected instrumentation failed with exit code $testExitCode."
        }
    }
    "Uninstall" {
        $device = Resolve-Device
        ...
        if (-not (Get-PackagePath $device $TestPackage)) {
            Write-Host "$TestPackage is not installed on $device."
            break
        }
        Invoke-Adb $device @("uninstall", $TestPackage)
        if (Get-PackagePath $device $TestPackage) {
            throw "D14 test package remains installed after uninstall."
        }
        Write-Host "Removed only $TestPackage. Production package $ProductionPackage was not targeted."
    }
}
