...
目前 `main` 尚未設定 required status checks；CI 會執行並回報結果，但 repository 管理員仍需把 `Windows verification` 設為必要檢查，才會阻擋不合格的合併。D01–D12 專屬回歸案例會隨各功能修復加入；現有 CI 測試尚不代表這些案例已全部涵蓋。
